import {
  CartesianGrid,
  Line,
  LineChart,
  ResponsiveContainer,
  Tooltip,
  XAxis,
  YAxis,
} from 'recharts';
import type { QuarterlyOverallPoint } from '@/types/churn';

type QuarterlyUsageChartProps = {
  quarterlyOverall: QuarterlyOverallPoint[];
};

const LINE_COLOR = '#466CFF';

export function QuarterlyUsageChart({
  quarterlyOverall,
}: QuarterlyUsageChartProps) {
  const firstPredictedIndex = quarterlyOverall.findIndex(
    (point) => point.predicted,
  );
  const chartRows = quarterlyOverall.map((point, index) => {
    const isConnectorPoint =
      firstPredictedIndex > 0 && index === firstPredictedIndex - 1;
    const isActualSegment = !point.predicted || isConnectorPoint;
    const isPredictedSegment = point.predicted || isConnectorPoint;

    return {
      quarter: point.quarter,
      actual: isActualSegment ? point.totalUsage : null,
      predicted: isPredictedSegment ? point.totalUsage : null,
    };
  });
  const quarterlyByQuarter = new Map(
    quarterlyOverall.map((point) => [point.quarter, point]),
  );

  return (
    <div className="h-64">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart
          data={chartRows}
          margin={{ top: 8, right: 8, bottom: 0, left: 0 }}
        >
          <CartesianGrid strokeDasharray="3 3" stroke="#f1f5f9" />
          <XAxis
            dataKey="quarter"
            tick={{ fontSize: 12, fill: '#9ca3af' }}
            axisLine={false}
            tickLine={false}
          />
          <YAxis
            tick={{ fontSize: 12, fill: '#9ca3af' }}
            axisLine={false}
            tickLine={false}
            width={56}
            tickFormatter={(value: number) => `${(value / 10000).toFixed(1)}억`}
          />
          <Tooltip
            content={({ active, label }) => {
              if (!active || typeof label !== 'string') {
                return null;
              }
              const point = quarterlyByQuarter.get(label);
              if (!point) {
                return null;
              }

              return (
                <div className="rounded-lg border border-white/40 bg-white/70 px-3 py-2 shadow-lg backdrop-blur-md">
                  <p className="text-xs font-medium text-gray-500">
                    {point.quarter}
                  </p>
                  <p className="mt-1 text-sm font-semibold text-gray-900">
                    {(point.totalUsage / 10000).toFixed(1)}억원
                  </p>
                </div>
              );
            }}
          />
          <Line
            dataKey="actual"
            stroke={LINE_COLOR}
            strokeWidth={2}
            dot={{ r: 3 }}
            connectNulls={false}
          />
          <Line
            dataKey="predicted"
            stroke={LINE_COLOR}
            strokeWidth={2}
            strokeDasharray="5 4"
            dot={{ r: 3 }}
            connectNulls={false}
          />
        </LineChart>
      </ResponsiveContainer>
    </div>
  );
}
